...
bpm_default = 120
division = 48                # 마디 분할 수
min_length_ms = 30           # 단노트 기준
base_lane = 11               # 첫 번째 MIDI 채널 (LN용)
...
for idx, (midi_path, wav_path) in enumerate(zip(midi_files, wav_files)):
    ...
    lane_channel = f"{base_lane + idx:02}"  # ex: 11,12,13 (LN용)
    ...
    # --- 마디별 배치 ---
    bar_duration = (60 / bpm_default) * 4  # 1마디 길이
    ...
    for start_sec, end_sec, wav_id, length_ms in event_list:
        ...
        if length_ms < min_length_ms:
            ...
            measure_data[start_measure][lane_channel][start_div] = f"{wav_id:02}"
        else:
            # LN 처리: 시작부터 끝까지 LN 연장 코드 '01' 사용
            ...
            while (measure_counter < end_measure) or (measure_counter == end_measure and pos_counter <= end_div):
                ...
                if (measure_counter == start_measure and pos_counter == start_div) or (measure_counter == end_measure and pos_counter == end_div):
                    # LN 시작/끝 칸에는 WAV 번호
                    measure_data[measure_counter][lane_channel][pos_counter] = f"{wav_id:02}"
                else:
                    # 중간 구간 LN 연장 코드
                    measure_data[measure_counter][lane_channel][pos_counter] = "01"
