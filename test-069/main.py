...
for idx, (midi_path, wav_path, inst_name) in enumerate(zip(midi_files, wav_files, instrument_names)):
    ...
    for i, start_sec in enumerate(notes):
        ...
        if key not in note_map:
            ...
            filename = os.path.join(
                output_dir, f"{inst_name}-{next_wav_index+1}.ogg")
            segment = audio[start_ms:start_ms+length_ms]
            segment.export(filename, format="ogg")  # ← ogg로 저장
    ...
    for key, idxnum in sorted(note_map.items(), key=lambda x: x[1]):
        ### (변경됨) — BMS에서도 .ogg 확장자 반영
        bms_lines.insert(
            insert_index, f"#WAV{to36(idxnum):02} {os.path.basename(output_dir)}/{inst_name}-{idxnum+1}.ogg")
...
print("🎉 모든 MIDI 병합 완료 (악기별 레인, 단노트, notes/*.ogg, 36진수 WAV 번호)")
