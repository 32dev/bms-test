...
bms_path = "output.bms"
bpm_default = 96
division = 48  # 마디당 분할 수
base_lane = 11  # 첫 번째 레인 번호 (11부터 시작)
min_note_ms = 50
...
def to36(n):
    if n < 36:
        ...
        return to36(n // 36) + digits36[n % 36]
...
for idx, (midi_path, wav_path, inst_name) in enumerate(zip(midi_files, wav_files, instrument_names)):
    ...
    lane_channel = f"{lane_mapping[inst_name]:02}"
    ...
    for i, tick_start in enumerate(notes):
        ...
        if key not in note_map:
            ...
            filename = os.path.join(output_dir, f"{inst_name}-{wav_num:02}.ogg")  # 🔹 OGG로 저장
            segment = audio[int(start_sec * 1000):int(start_sec * 1000) + length_ms]
            segment.export(filename, format="ogg")  # 🔹 OGG 포맷
    ...
    for length_ms, wav_id in sorted(note_map.items(), key=lambda x: x[1]):
        ...
        if wav36 not in existing_wavs:
            file_num = instrument_file_counter[inst_name] - 1
            filename = f"{output_dir}/{inst_name}-{file_num:02}.ogg"
...
print("✅ 모든 MIDI 병합 완료!")
print("🎧 OGG 파일 생성 완료 및 kick/snare 포함 BMS 완성!")
