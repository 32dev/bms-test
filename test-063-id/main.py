...
for idx, (midi_path, wav_path) in enumerate(zip(midi_files, wav_files)):
    ...
    for track in mid.tracks:
        ...
        for msg in track:
            ...
            if msg.type == "note_on" and msg.velocity > 0:
                notes.append((tick_to_sec(current_tick), msg.note))

    notes.sort(key=lambda x: x[0])

    # --- WAV 추출 및 중복 방지 ---
    note_map = {}  # note -> WAV 번호
    event_list = []

    for i, (start_sec, note) in enumerate(notes):
        end_sec = notes[i+1][0] if i + \
            1 < len(notes) else tick_to_sec(mid.length)
        ...
        if key not in note_map:
            filename = os.path.join(
                output_dir, f"note_{to36(next_wav_index):02}.wav")
    ...
    # --- WAV 등록 ---
    insert_index = next((i for i, l in enumerate(bms_lines) if l.startswith(
        "*---------------------- MAIN DATA FIELD")), len(bms_lines))
    for key, idxnum in sorted(note_map.items(), key=lambda x: x[1]):
        bms_lines.insert(
            insert_index, f"#WAV{to36(idxnum):02} {os.path.basename(output_dir)}/note_{to36(idxnum):02}.wav")
...
print("🎉 모든 MIDI 병합 완료 (단노트, WAV 길이 기준, 36진수 번호)")
