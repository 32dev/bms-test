...
for idx, (midi_path, wav_path) in enumerate(zip(midi_files, wav_files)):
    ...
    base_name = os.path.splitext(os.path.basename(midi_path))[0]  # input1 등
    ...
    for start_sec, end_sec, note in notes:
        ...
        if key not in note_map:
            filename = os.path.join(output_dir, f"{base_name}-{local_index:02}.wav")
            segment = audio[start_ms:end_ms]
            segment.export(filename, format="wav")
            wav_id = next_wav_index
            note_map[key] = (wav_id, f"{base_name}-{local_index:02}.wav")
            next_wav_index += 1
            local_index += 1
        else:
            wav_id, _ = note_map[key]

        # BMS에는 시작 위치에만 WAV 번호 등록
    ...
    for _, (idxnum, filename) in sorted(note_map.items(), key=lambda x: x[1][0]):
        bms_lines.insert(insert_index, f"#WAV{idxnum:02} {os.path.basename(output_dir)}/{filename}")

    # --- 마디별 배치 (시작 위치만 WAV 번호) ---
    ...
    for start_sec, wav_id in event_list:
        ...
        measure_data[measure][lane_channel][div] = f"{wav_id:02}"
