...
# === 설정 ===
midi_path = "song.mid"
wav_path = "song.wav"      # 전체 song.wav를 BGM으로 사용
output_dir = "notes"
bms_path = "song.bms"
bma_path = "song.bma"
...
lane_map = {note: (i % 15) + 1 for i, note in enumerate(unique_notes)}
...
# song.wav를 BGM으로 등록 (WAV00)
header.append(f"#WAV00 {wav_path}")
...
# BGM 배치 (B0 채널, WAV00)
bms_lines.append("#00001:B0:00")  # 한 마디 시작에 단일 WAV00
...
for note, evs in note_events.items():
    ...
    for ev in evs:
        ...
        position = min(max(position, 0), 191)

        wav_id = wav_id_map.get((note, ev["start_tick"]), "01")
        # 노트 하나마다 최소 단위 문자열 생성
        note_line = f"#{measure:03d}{channel}:{'00'*position}{wav_id}"
        bms_lines.append(note_line)
