...
# === 실제 존재하는 노트만 순서대로 WAV ID 매기기 (01~) ===
note_to_id = {}
valid_notes = [note for note, times in note_segments.items() if len(times) >= 2]
for idx, note in enumerate(sorted(valid_notes), start=1):
    note_to_id[note] = idx

# === note별 오디오 자르기 + BMS 데이터 준비 ===
bms_data = []

# measure 단위로 동시음 lane 할당 관리
lane_map = defaultdict(dict)  # measure -> lane 사용 현황
...
for note, times in note_segments.items():
    ...
    # lane 자동 할당 (동시음 처리)
    measure_lanes = lane_map[measure]
    lane = 1
    while lane in measure_lanes and abs(measure_lanes[lane] - position) < 2:  # 겹침 방지
        lane += 1
    measure_lanes[lane] = position
